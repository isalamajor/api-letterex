    ...
    try {
        ...
        if (!name || !language || !description || isPrivate === null) {
            return res.status(400).json({
                status: 'error',
                message: 'Fields missing. Required fields: name, language, description, isPrivate'
            })
        }
        ...
        if (
            ...
            return res.status(400).json({
                status: "error",
                message: "Invalid input: name, language, description must be non-empty strings and private must be boolean"
            });
        }
...
const getCommunities = async (req, res) => {
    try {
        ...
        const communitesExpanded = communites.map(community => {
            ...
            const loggedUserSentRequest = !!community.pendingRequests.find(request => request.user.equals(userId))
            const loggedUserIsMember = !!community.members.find(request => request.user.equals(userId))
            return { ...c, creator: c.creator.nickname, members: c.members.length, id: c._id.toString(), _id: undefined, userIsMember: loggedUserIsMember, userSentRequest: loggedUserSentRequest }
        })
...
const getUserCommunities = async (req, res) => {
    try {
        ...
        const communitesExpanded = communites.map(community => {
            const c = community.toObject()
            return { ...c, creator: c.creator.nickname, members: c.members.length, id: c._id.toString(), _id: undefined }
        })
...
const updateCommunity = async (req, res) => {
    try {
        ...
        if (!communityId || !(description || isPrivate)) {
            return res.status(400).json({
                status: 'error',
                message: 'Missing fields. Fields required in url: communityId, and descripcion, isPrivate or both'
            })
        }
...
const uploadCommunityPicture = async (req, res) => {
  try {
    ...
    const oldFiles = files.filter(f => f.startsWith(communityId) && f !== req.file.filename);
...
const deleteCommunityPicture = async (req, res) => {
  try {
    ...
    const imagePath = path.resolve(`./uploads/community_pictures/${community.image}`);
        try {
