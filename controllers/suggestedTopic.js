  ...
  try {
    ...
    if (!communityId || !title || !description) {
      return res.status(400).json({
        status: "error",
        message:
          "Missing fields. Fields requiered: communityId, title, description",
      });
    }
...
const deleteTopic = async (req, res) => {
  try {
    ...
    const community = await Community.findById(topic.community).select(
      "creator",
    );
  ...
  try {
    ...
    if (community.members.filter((m) => m.user.equals(userId)).lenth === 0) {
      return res.status(401).json({
        status: "error",
        message:
          "User is not part of this community so they cannot perform this action",
      });
    }

    const topics = await SuggestedTopic.find({
      community: communityId,
